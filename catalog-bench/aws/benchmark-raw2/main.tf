...
resource "aws_instance" "ycsb_vm" {
  ...
  user_data = <<-EOF
    ...
    apt-get update
    apt-get install -y --no-install-recommends build-essential ca-certificates curl dbus-user-session git gnupg jq lsb-release openjdk-17-jdk-headless python3-pip software-properties-common ssh unzip wget
